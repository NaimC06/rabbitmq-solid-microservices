...
IMessageHandler<SecurityAlert> handler = new SecurityAlertHandler();

await topologyInitializer.InitializeAsync(cancellationTokenSource.Token);

Console.ForegroundColor = ConsoleColor.Yellow;
Console.WriteLine("=== SECURITY CONSUMER SERVICE ===");
Console.ResetColor();

await consumer.StartConsumingAsync(QueueNames.SecurityTeam, handler, cancellationTokenSource.Token);
